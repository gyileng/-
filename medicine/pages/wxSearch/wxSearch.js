...
function wxsearchGoogle(that){
  ...
  if (typeof (text) == "undefined" || text.length == 0) {
    ...
    console.log(text)
    wx.request({
      url: 'https://www.googleapis.com/customsearch/v1?key=AIzaSyDf06axzNTAYKG9AFmLVmNPKAY4hUSSs4g&cx=000965262837307580461:eooganbttgq&q=' + text,
      success: function (res) {
        var jsonData = that.data.googleData
        jsonData.value = res.data.items
        that.setData({
          googleData: jsonData
        });
      }
